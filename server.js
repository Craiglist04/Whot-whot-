...
function startTurnTimer(code) {
  ...
  turnTimers[code] = setTimeout(() => autoPlay(code), 8000);
}
...
function settleStake(room) {
  if (!room.stake || room.stake <= 0 || !room.winner) return;
  const pot = room.stake * room.players.length;
  const users = loadUsers();
  const winP = room.players.find(p => p.name === room.winner);
  if (winP && winP.username && users[winP.username]) {
    users[winP.username].balance = (users[winP.username].balance || 0) + pot;
    saveUsers(users);
    room.effect += " · " + winP.name + " wins pot ₦" + pot.toLocaleString();
    room.players.forEach(p => {
      const sock = io.sockets.sockets.get(p.id);
      if (sock && users[p.username]) sock.emit("auth_ok", publicUser(users[p.username]));
    });
  }
}
...
io.on("connection", (socket) => {
  ...
  socket.on("verify", ({ username, code }) => {
    ...
    if (users[u].verified) {
      ...
      return socket.emit("auth_ok", publicUser(users[u]));
    }
  ...
  socket.on("login", ({ username, password }) => {
    ...
    socketUser[socket.id] = u;
    socket.emit("auth_ok", publicUser(acc));
  });
